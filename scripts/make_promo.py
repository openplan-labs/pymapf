...
def disc(ax, x, y, radius, **kwargs):
    ...
    return Ellipse(
        (x, y), 2 * radius, 2 * radius * ASPECT, transform=ax.transAxes, **kwargs
    )


def draw_agent(
    ax, grid, origin, cell, position, color, radius=0.4, alpha=1.0, label=None
):
...
class Content:
    def __init__(self):
        ...
        self.old = pymapf.build_scenario(
            "empty_room", height=10, width=10, n_agents=3, seed=2
        )
        ...
        self.success = {
            name: 100 * self.report.success_rate(name)
            for name in self.report.algorithms
        }
...
def scene_title(ax, t, content):
    ...
    for index, name in enumerate(names):
        ...
        draw_agent(
            ...
            origin,
            cell,
            interpolate(paths[name], progress),
            color,
            radius=0.3,
            alpha=0.5 * ease_out(t / 0.8),
        )
    ...
    text(
        ...
        0.5,
        0.48,
        "multi-agent path finding you can watch",
        size=25,
        color=THEME.ink_secondary,
        ha="center",
        alpha=fade(ax, t, 0.9, 0.7),
    )
    text(
        ax,
        0.5,
        0.38,
        "openplan-labs/pymapf",
        size=15,
        color=THEME.muted,
        ha="center",
        family=MONO,
        alpha=fade(ax, t, 1.4, 0.7),
    )
...
def scene_origin(ax, t, content):
    ...
    for index, (line, color) in enumerate(lines):
        ...
        text(
            ax, 0.10, 0.62 - 0.075 * index, "—", size=17, color=THEME.axis, alpha=alpha
        )
...
def scene_search(ax, t, content):
    ...
    text(
        ...
        0.07,
        0.835,
        "Every solver streams its own events. Nothing here is a mock-up.",
        size=16,
        color=THEME.ink_secondary,
        alpha=fade(ax, t, 0.25),
    )
    ...
    if solved:
        ...
        text(
            ...
            panel_x,
            0.31,
            "conflict-free · optimal",
            size=19,
            color=theme_module.STATUS["good"],
            weight="bold",
            alpha=fade(ax, t, 0.0, 0.3),
        )
        text(
            ax,
            panel_x,
            0.25,
            "sum of costs %d   ·   makespan %d"
            % (solution.sum_of_costs, solution.makespan),
            size=15,
            color=THEME.ink_secondary,
        )
...
def scene_execute(ax, t, content):
    ...
    for index, (name, path) in enumerate(solution.paths.items()):
        ...
        tail = max(0, head - 8)
        draw_path(
            ax, grid, origin, cell, path[tail : head + 1], color, alpha=0.55, width=2.6
        )
        draw_agent(
            ax,
            grid,
            origin,
            cell,
            interpolate(path, min(clock, len(path) - 1)),
            color,
            radius=0.36,
            label=name,
        )
    ...
    text(
        ...
        0.78,
        0.48,
        "t = %d / %d" % (int(min(clock, solution.makespan)), solution.makespan),
        size=22,
        family=MONO,
        color=THEME.ink_secondary,
    )
...
def scene_swarm(ax, t, content):
    ...
    text(
        ...
        0.07,
        0.90,
        "The other half: no planner at all.",
        size=34,
        weight="bold",
        alpha=fade(ax, t, 0.0),
    )
    ...
    if t > 3.1:
        text(
            ...
            0.07,
            0.17,
            "polarisation  %.2f" % content.flock_order,
            size=25,
            weight="bold",
            family=MONO,
    ...
    text(
        ...
        0.07,
        0.10,
        "Amorim, Nascimento, Chaudhary, Ferrante, Saska (2024)",
        size=12.5,
        color=THEME.muted,
        alpha=fade(ax, t, 3.4),
    )
...
def scene_formation(ax, t, content):
    ...
    text(
        ...
        0.07,
        0.90,
        "Formation control: hold a shape.",
        size=34,
        weight="bold",
        alpha=fade(ax, t, 0.0),
    )
    ...
    for agent in range(positions.shape[0]):
        ...
        ax.add_patch(
            disc(
                ...
                y,
                0.0082,
                facecolor=THEME.agent_color(agent),
                edgecolor=THEME.plane,
                linewidth=1.0,
                alpha=ease_out(t / 0.7),
                zorder=5,
            )
        )
    ...
    text(
        ax, 0.79, 0.70, "free up to", size=13, color=THEME.muted, alpha=fade(ax, t, 1.0)
    )
...
def scene_benchmark(ax, t, content):
    ...
    text(
        ...
        0.07,
        0.90,
        "Three solvers. Measured, not asserted.",
        size=34,
        weight="bold",
        alpha=fade(ax, t, 0.0),
    )
    ...
    for decade in range(int(log_lo), int(log_hi) + 1):
        ...
        text(
            ax, px - 0.012, y, fmt(10.0**decade), size=11, color=THEME.muted, ha="right"
        )
    for value in xs:
        text(
            ax,
    ...
    for order, name in enumerate(sorted(runtime)):
        ...
        points = [
            (to_x(x), to_y(y)) for x, y in zip(series_x[:count], series_y[:count])
        ]
        if len(points) < 2:
            continue
        ax.plot(
        ...
        ends.append(label_y)
        text(
            ax, points[-1][0] + 0.014, label_y, name, size=14, color=THEME.ink_secondary
        )
...
def scene_outro(ax, t, content):
    ...
    text(
        ...
        0.5,
        0.55,
        "solvers · scenarios · benchmarks · live views",
        size=21,
        color=THEME.ink_secondary,
        ha="center",
        alpha=fade(ax, t, 0.5),
    ...
    text(
        ...
        0.5,
        0.30,
        "Apache 2.0 · a university project, rebuilt",
        size=14,
        color=THEME.muted,
        ha="center",
        alpha=fade(ax, t, 1.3),
...
def build_animation(content, seconds=None):
    ...
    animation = FuncAnimation(
        figure, render, frames=frames, interval=1000 // FPS, blit=False
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--preview", type=float, default=None, help="render only N seconds"
    )
    parser.add_argument(
        "--dpi", type=int, default=80, help="80 -> 1280x720, 120 -> 1920x1080"
    )
